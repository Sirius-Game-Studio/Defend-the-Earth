...
public class GameController : MonoBehaviour
{
    ...
    private long wave = 1;
    private int enemyAmount = 0; //Stores the enemiesLeft value
    private int clickSource = 1; //1 is game paused menu, 2 is game over menu, 3 is level completed menu
    private bool check = false; //Checks if the player is on the next wave, ensuring the wave doesn't skyrocket to the last wave
    private bool loading = false;

    void Awake()
    {
        PlayerPrefs.SetString("Money", "999999");
        PlayerPrefs.Save();
        if (instance == null)
        {
            instance = this;
        ...
        //Destroy all player objects that contain PlayerController
        GameObject[] players = GameObject.FindGameObjectsWithTag("Player");
        foreach (GameObject player in players)
        {
            if (player && player.GetComponent<PlayerController>()) Destroy(player);
        }
        ...
        if (!gameOver && !won && enemiesLeft <= 0)
        {
            ...
            if (wave < maxWaves)
            {
                if (!check)
                {
                    check = true;
                    ++wave;
                    if (wave >= maxWaves + 1) completed = true;
                }
            } else if (wave >= maxWaves && completed)
            {
                won = true;
                clickSource = 3;
